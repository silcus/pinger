...
void *child_loop(void *data)
{
    ...
    while (host_info != NULL) {
        if (host_info->rawfd > fd_max)
            fd_max = host_info->rawfd;
    ...
    while ((*parent_living == 1) && (stop_gui_loop == 0)) {
        ...
        if (((selected =
              select(fd_max + 1, &fdset, NULL, NULL, &cur_timeout)) < 0)
            && (errno != EINTR)) {
            log_event(NULL, err_log, -1, "%s %s", _("Cannot select:"),
                      strerror(errno));
            exit(1);
        }
        ...
        if ((*parent_living == 0) || (stop_gui_loop == 1))
            break;
...
{
    if (strcmp(inet_ntoa(addr), UNRESOLVED_HOST) == 0)
        return "---";
    else
...
void set_virt_host(host_data * host, host_data * virt_host)
{
    ...
    if (host->like != NULL) {
        ...
        memcpy(virt_host, host, sizeof(host_data));
}
...
#error There must be some defined GUI mode
    ...
    if (MAX_CFGLINE_LEN < MAX_TITLE_LEN) {
        log_event(NULL, err_log, -1, "%s %s!! %s", "MAX_TITLE_LEN",
                  _("must be less than"), "MAX_CFGLINE_LEN",
                  ("Someone doesn't know how to make changes in the source code!"));
        exit(1);
    }
    if (strlen(STD_TITLE) > MAX_TITLE_LEN) {
        log_event(NULL, err_log, -1, "%s %s!! %s", _("must be greater than"),
                  "MAX_TITLE_LEN",
                  _("Someone doesn't know how to make changes in the source code!"));
        exit(1);
    }
    if (MIN_TITLE_LEN > MAX_TITLE_LEN) {
        log_event(NULL, err_log, -1, "%s %s!! %s", "MIN_TITLE_LEN",
                  _("must be greater than"), "MAX_TITLE_LEN",
                  _("Someone doesn't know how to make changes in the source code!"));
        exit(1);
    }
    ...
    if (argc > 1) {
        for (a = 1; a < argc; a++) {
            if (!strcmp(argv[a], "--gtk")) {
                ...
                printf("\n%s\n", _("Usage:"));
                printf
                    ("    pinger [--gtk] [--quiet] [conffile1] [conffile2] ..\n\n");
                printf("%s\n", _("Parameters:"));
                printf("    --gtk   : %s\n",
...
please recompile this program with enabling ncurses mode."));
    ...
    if (seteuid(saved_euid) != 0) {
        log_event(NULL, err_log, -1, "%s",
                  _("Cannot restore back root privileges, quiting."));
        exit(1);
    }
...
#ifdef ENABLE_ARP
    if (seteuid(saved_euid) != 0) {
        log_event(NULL, err_log, -1, "%s %s", _("ARP code"),
                  _("Cannot set root privileges, quitting."));
        exit(1);
    }
    get_ifaces();
    if (seteuid(getuid()) != 0) {
    ...
    if ((hosts.icmp_pkt_len =
         allocate_icmp_packet(&(hosts.icmp_pkt), hosts.icmp_data_len)) == -1) {
        log_event(NULL, err_log, -1, "%s",
                  _("Cannot allocate icmp packet (bad icmp 'packetsize' perhaps?)."));
        exit(1);
    }
