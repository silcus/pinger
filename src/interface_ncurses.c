                      ...
                      char *lastokstr, int host_set_nr)
{
    ...
    if ((host_displayed_cnt < (win_sizey - 3)) && (host->set_nr == host_set_nr)) {
        host_displayed_cnt++;
        count_line_pos(&curseswin, &host_pos, &delay_pos, &name_pos, &stat_pos,
                       &sr_pos, &avail_pos, &lastok_pos);
        ...
        if (lastok_pos > 0)
            mvprintw(y_pos, lastok_pos, "%s", lastokstr);
        attroff(attr);
        /* print time */
...
int ncurses_show_status(host_data * host)
{
    ...
    if ((h->delay > MAX_DELAY) || (h->delay < 0))
        sprintf(number_str, "%s", _("a lot!"));
    else
        snprintf(number_str, sizeof(number_str) - 1, "%4.2f", h->delay);
    ...
    if (h->nr_recv > 0) {
        ...
        if (h->lastok_tv.tv_sec > 0) {
            ...
            if ((timenow.tm_mday != timeok.tm_mday) ||
                ...
                strftime(lastok_str, sizeof(lastok_str), "%b %d %H:%M",
                         &timeok);
            else
                sprintf(lastok_str, "%.2d:%.2d:%.2d", timeok.tm_hour,
                        timeok.tm_min, timeok.tm_sec);
        } else
            strcpy(lastok_str, "-");
    } else {
        if (mode == NCURSES)
    ...
    case ECHO_OK:
        if (mode == NCURSES)
            show_host_status(h, COL_ONLINE, A_BOLD, "Online", number_str,
                             sr_str, avail_str, lastok_str, get_curr_set_nr());
        break;
    case DELAYED:
        if (mode == NCURSES)
            show_host_status(h, COL_DELAYED, A_BOLD, "Online", number_str,
                             sr_str, avail_str, lastok_str, get_curr_set_nr());
        break;
    case NO_REPLY:
        if (mode == NCURSES)
            show_host_status(h, COL_OFFLINE, A_NORMAL, "Offline", number_str,
                             sr_str, avail_str, lastok_str, get_curr_set_nr());
        break;
    case NW_UNREACH:
        if (mode == NCURSES)
            show_host_status(h, COL_NWUNREACH, A_NORMAL, _("NwUnrch"),
                             number_str, sr_str, avail_str, lastok_str,
                             get_curr_set_nr());
        break;
    case UNRESOLVED:
        if (mode == NCURSES)
            show_host_status(h, COL_NWUNREACH, A_NORMAL, _("Resolving"),
                             number_str, sr_str, avail_str, lastok_str,
                             get_curr_set_nr());
        break;
    }
...
//if (title != NULL) free(title);
    ...
    if (lastok_pos > 0)
        mvprintw(1, lastok_pos, _("Last OK"));
    attroff(A_BOLD);
    refresh();
}
...
int get_set_refreshint(int set_nr, titlelist * tlist)
{
    ...
    if (tl == NULL)
        refresh_int = DEFAULT_REFRESH_INTERVAL;
    else
        refresh_int = tl->refresh_int;
...
int update_header(hosts_data * hosts)
{
    ...
    titles = hosts->titles;
    while ((titles->next != NULL) && (titles->nr != set_nr))
        titles = titles->next;
...
void adjust_size()
{
    ...
    if ((!size_changed) && (ioctl(fileno(stdout), TIOCGWINSZ, &size) == 0)) {
        new_cols =
            (size.ws_col < MIN_LINE_WIDTH) ? MIN_LINE_WIDTH : size.ws_col;
