...
int check_byte(char *byte) {
  ...
  byte_val = atoi(byte);
  if ((byte_val < 0) || (byte_val > 255))  return 1;
  else return 0;
}
...
int check_opt_value(char * value) {
  ...
  intval = atoi(value);
  if ((intval < 0) || (intval > MAX_OPT_VALUE)) return 1;
  else return 0;
...
int valid_dname(char *name) {
  ...
  for (cnt = 0; cnt < strlen(name); cnt++) {
    ...
    if (!(((ch >= '0') && (ch <= '9')) || ((ch >= 'a') && (ch <= 'z')) ||
      ...
      for (a = 0; (a < sizeof(dns_allowed_chars)) && (ch != dns_allowed_chars[a]); a++);
...
int valid_ip(char *ip) {
  ...
  byte_cnt = 0;
  if ((len = strlen(ip)) <= 15) {
    for (cnt = 0, dot_cnt = 0, byte_cnt = 0,
        exit_now = 0; cnt < len; cnt++) {
      if (ip[cnt] == '.') {
        if (byte_cnt == 0) exit_now = 1;
        else {
          byte[byte_cnt] = '\0';
          if (check_byte(byte)) exit_now = 1;
          byte_cnt = 0;
        }
        dot_cnt++;
      } 
      else if (((int)ip[cnt] < (int)'0') || 
          ((int)ip[cnt] > (int)'9')) exit_now = 1;
      else {
        byte[byte_cnt] = ip[cnt];
        if (++byte_cnt == 4) exit_now = 1;
      }
      if (exit_now) cnt = len;
    }
    if (dot_cnt != 3) exit_now = 1;
  } else exit_now = 1;
...
int is_num(char *s) {
  ...
  l = strlen(s);
  while (--l >= 0) 
    if (((s[l] < 48) || (s[l] > 57)) && (s[l] != ' ') && (s[l] != 9)) return 0;
  return 1;
}
...
void read_config_file(hosts_data *hosts, filelist *cfg_filenames) {
  ...
  while (!end_parsing) {
    ...
    if (!cfg_file) {
      ...
      if (cfgfilesidx != NULL) {
        ...
        if (cfgfilesidx == NULL) end_parsing =1;
        else strncpy(cfg_path, cfgfilesidx->filename, sizeof(cfg_path) - 1);
        break;
      }
      qprint("--\n%s\n", _("Creating new one in your home directory ..."));
      cfg_file = fopen(cfg_path, "w");
      if (!cfg_file) {
        qprint("%s. %s\n", _("Cannot create file"), _("Please check your create privileges for this directory!"));
        exit(1);
      } else {
        fprintf(cfg_file, "# %s\n", _("Pinger configuration file"));
        fprintf(cfg_file, "# <IP or domain name>=<%s>[,arp][,[no]log][,ok_delay,noreply_delay,long_delay]\n", _("Description"));
        ...
        fprintf(cfg_file, "\n# %s %% %s\n", _("Title of this set of hosts. Character"), _("is not allowed here!"));
        ...
        if (chown(cfg_path, getuid(), getgid()))
          qprint("%s %s\n", _("Cannot set configuration file owner to current user:"), strerror(errno));
        cfg_file = fopen(cfg_path, "r");
        if (!cfg_file) {
          qprint("%s\n", _("Cannot reopen just created config file!"));
          exit(1);
        }
        qprint("%s (%s)\n", _("Config file created successfuly"),cfg_path);
      }
    ...
    /* read from cfg file */
    title_set = 0;
    while ((fgets(long_line, LONG_CFGLINE_LEN, cfg_file) != NULL ) && (host_max < MAX_HOSTS)) {
        ...
        if (strcmp("OPTIONS SECTION:\n", line) == 0) {
          ...
          if (section == HOSTS) {
            ...
            if (valid_ip(name)) {
              if (valid_dname(name)) {
                ...
                if (strlen(name) > MAX_DNAME_LEN) {
                  log_event(NULL, err_log, -1, _("Domain name is too long! Update this program source or check if domain name is OK!"));
                  exit(1);
                }
                name_type = DNAME;
              }
            ...
            if (host_ptr != NULL) {
              ...
              if (name_type == DNAME) {
                while (host_ptr != NULL) {
                  if (strcmp(host_ptr->domain_name, name) == 0) break;
                  host_ptr = host_ptr->next;
                }
              } else {
                /* else check IPs */
                while (host_ptr != NULL) {
                  if (strcmp(inet_ntoa(host_ptr->addr), name) == 0) break;
                  host_ptr = host_ptr->next;
                }
              }
            }
            ...
            if (host_ptr != NULL) {
              ...
              } else strcpy(host_info->domain_name, "");
              /* add address into struct */
              if (inet_aton(name, &(host_info->addr)) == 0) {
                log_event(NULL, err_log, -1, "%s %s!", _("Invalid host"), host_info->name);
                exit(1);
              }
              ...
              if (parse_value(value, val_name, &ok_delay, &noreply_delay, &long_delay, &use_arp, &log)) 
                qprint("Cannot parse line\n");
                
              if (val_name != NULL) qprint("%s: %s\n", _("Found name"), val_name);
              strcpy(host_info->name, val_name);
              if (ok_delay != 0) {
                qprint("%s: %d ms\n", _("Setting ok delay"), ok_delay);
                host_info->ok_delay = ok_delay;
              } 
              if (noreply_delay != 0) {
                qprint("%s: %d ms\n", _("Setting noreply delay"), noreply_delay);
                host_info->noreply_delay = noreply_delay;
              } 
              if (long_delay != 0) {
                qprint("%s: %d ms\n", _("Setting long delay"), long_delay);
                host_info->long_delay = long_delay;
              }
              if (use_arp == 1) {
                qprint("%s\n", _("This host will use ARP request instead of ICMP ECHO"));
                host_info->arp = 1;
              } else host_info->arp = 0;
              if ((log != LOG_DEFAULT) && (host_log->log != NULL)) {
                if (log == LOG_DISABLED) {
                  qprint("%s\n", _("This host will not be logged in host log"));
                  host_info->log_if_status_last = LOG_DISABLED;
                } else {
                  qprint("%s\n", _("This host will be logged in host log"));
                  host_info->log_if_status_last = log_if_status_last;
                }
              } else host_info->log_if_status_last = LOG_DEFAULT;


              qprint("%s %s - %s\n", _("Adding host"), name, host_info->name);
...
#ifdef ENABLE_ARP
              if (host_info->arp) 
                host_info->rawfd = socket(PF_PACKET, SOCK_RAW, htons(ETH_P_ARP));
              else 
#endif
                (host_info->rawfd = socket(PF_INET, SOCK_RAW, IPPROTO_ICMP));
              
              /* restore users rights */
              seteuid(getuid());

              if (host_info->rawfd < 0) {
                log_event(NULL, err_log, -1, "%s: %s\n%s", _("Initializing socket"), strerror (errno), _("Make sure you have root privileges or suid bit set!"));
                exit(1);
              } 

              if (host_info->rawfd <= 2) {
                log_event(NULL, err_log, -1, _("Got free socket less than 2!"));
                exit (1); 
              }
            ...
            qprint("%s: %s, %s: %s\n", _("Name"), name, _("value"), value);
            /* Process options */
            if ((!check_opt_value(value)) || (strcmp(value, "-1") == 0)) {
              int_value = atoi(value);

              /* options are processed here */
              if (!strcmp(name, "dns_check_s")) {
                if ((int_value != -1) && 
                  (int_value >= DNS_CHECK_MIN_S) && (int_value <= DNS_CHECK_MAX_S)) {
                  dns_check_s = int_value;
                  qprint("%s: %s ms\n", _("Setting dns check to new value"), value);
                } else {
                  log_event(NULL, err_log, -1, "dns_check_s %s (%d-%d). %s %d.", _("is out of range"), 
                     DNS_CHECK_MIN_S, DNS_CHECK_MAX_S, _("Setting default value of"), dns_check_s);
                }
              } else if (!strcmp(name, "log_if_status_last_s")) {
                if ((int_value < MIN_STATUS_LAST) || (int_value > MAX_STATUS_LAST)) {
                  log_event(NULL, err_log, -1, "log_if_status_last_s %s (%d-%d). %s %d.", _("is out of range"), 
                      MIN_STATUS_LAST, MAX_STATUS_LAST, _("Setting default value of"), DEFAULT_STATUS_LAST);
                ...
                qprint("%s: %d s\n", _("Setting logging when host status last longer than value of (seconds)"), int_value);
              } else if (!strcmp(name, "icmp_packet_size")) {
                if ((int_value < MIN_ICMP_DATA_LEN) || (int_value > MAX_ICMP_DATA_LEN)) {
                  log_event(NULL, err_log, -1, "icmp_packet_size %s (%d-%d). %s %d.", _("is out of range"), 
                      MIN_ICMP_DATA_LEN, MAX_ICMP_DATA_LEN, _("Setting default value of"), DEFAULT_ICMP_DATA_LEN);
                  int_value = DEFAULT_ICMP_DATA_LEN;
                }
                qprint("%s: %d\n", _("Setting ICMP packet size to value"), int_value);
                ...
                qprint("%s: %d ms\n", _("Setting refresh interval to new value"), int_value);
              } else if (!strcmp(name, "host_timeout_ms")) {
                set_noreply_delay = int_value;
                qprint("%s: %d ms\n", _("Setting noreply interval to new value"), int_value);
              } else if (!strcmp(name, "delay_between_pings_ms")) {
                set_ok_delay = int_value;
                qprint("%s: %d ms\n", _("Setting delay between pings to new value"), int_value);
              } else if (!strcmp(name, "long_delay_ms")) {
                set_long_delay = int_value;
                qprint("%s: %d ms\n", _("Setting long delay to new value"), int_value);
              } 

            } else log_event(NULL, err_log, -1, "%s '%s'", _("Cannot evaluate OPTIONS value of"), value);

            if (!strcmp(name, "title")) {
              qprint("%s: %s\n", _("Setting title to new value"), value);
              if (recent_title == NULL) {
                recent_title = malloc(strlen(value) + 1);
              } else {
                if (strlen(recent_title) < strlen(value)) 
                  if (realloc(recent_title, strlen(value) + 1) == NULL) {
                    log_event(NULL, err_log, -1, "%s, %s:%d", _("Realloc failed"), __FILE__, __LINE__);
                  }
              }
              strcpy(recent_title, value);
                  
            }
            if ((!strcmp(name, "err_log")) || (!strcmp(name, "host_log"))) {
              if (!strcmp(name, "err_log")) {
                qprint("%s: %s\n", _("Setting error log to new value"), value);
                log_error = init_log(err_log, value, _("Error log"), set_nr);
              } else {
                qprint("%s: %s\n", _("Setting host log to new value"), value);
                log_error = init_log(host_log, value, _("Host status change log"), set_nr);
              }
              if (log_error != LOG_OK) {
                if (log_error == LOG_BAD_FILE)
                  log_event(NULL, err_log, -1, "%s: %s", _("Cannot open file"), strerror(errno));
                else
                  log_event(NULL, err_log, -1, "%s", log_error_str);
              }
            }
    ...
    if (host_ininitalized_curr_file > 0) {
      ...
      log_ena = 0; log_dis = 0; log_def = log_if_status_last;
      while (host_ptr != NULL) {
        if (host_ptr->set_nr == set_nr) {
          if (host_ptr->log_if_status_last == LOG_DISABLED) log_dis++;
          if (host_ptr->log_if_status_last >= 0) log_ena++;
        }
        host_ptr = host_ptr->next;
      }
      ...
      /* Otherwise use default value :-) */
        ...
        if (log_if_status_last == DEFAULT_STATUS_LAST) {
          qprint("%s ", _("Not specified hosts will use default setting:"));
        } else {
          qprint("%s ", _("Not specified hosts will use gobal setting:"));
        }
        if (log_if_status_last == -1) {
          ...
          qprint("%s (%s %d %s).\n", _("log"), _("with status last longer than"), 
      ...
      host_ptr = hosts->host_info;
      while (host_ptr != NULL) {
        if ((host_ptr->set_nr == set_nr) && (host_ptr->log_if_status_last == LOG_DEFAULT)) 
    ...
    if (cfg_filenames == NULL) end_parsing = 1;
    if (cfgfilesidx != NULL) {
      cfgfilesidx = cfgfilesidx->next;
      if (cfgfilesidx == NULL) end_parsing = 1;
      else strncpy(cfg_path, cfgfilesidx->filename, sizeof(cfg_path) - 1);
    ...
    if (host_ininitalized_curr_file > 0) {
      ...
      if ((tlist->title == NULL) && (strlen(recent_title) > MAX_TITLE_LEN)) {
        tlist->title = malloc(MAX_TITLE_LEN + 1);
        strncpy(tlist->title, recent_title, MAX_TITLE_LEN - 2 - stdcntlen);
        tlist->title[MAX_TITLE_LEN - 3 - stdcntlen] = 0;
        strcat(tlist->title, "..");
        if (stdcntlen) strcat(tlist->title, tmpstr);
      } else {
      ...
      if (tlistcnt >= MAX_TABS) {
        log_event(NULL, err_log, -1, _("Warning: maximum allowed configuration files exceeded!"));
        end_parsing = 1;
      }
      free(recent_title);
      recent_title = NULL;
  ...
  if (host_max == MAX_HOSTS) log_event(NULL, err_log, -1, "%s %d %s", 
  ...
  if (cfg_file_opened) fclose(cfg_file);
  qprint("------------------\n");
}
